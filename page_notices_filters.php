<?php
    ...
    if(isset($_GET['submit'])) {
        ...
        $ramSize = $_GET['ramSize'];
        $os = $_GET['os'];
        $damage = $_GET['damage'];

        if(isset($brand) && isset($cpuCores) && isset($ramSize) && isset($os) && isset($damage)) {
            $sql = "SELECT count(laptopID) as recCount FROM users, laptops WHERE users.userID = laptops.userID AND brand = $brand AND cpuCores = $cpuCores AND ramSize = $ramSize AND os = $os AND damage = $damage";
            $stmt = $pdo->query($sql);
            $record = $stmt->fetch();
            ...
            header('Location: page_notices.php?msg=Συμπληρώστε όλα τα πεδία.');
            exit();
        }
    } else {
